...
def get_stable_hwid():
    """
    Tạo HWID bằng cách sử dụng ID Android độc nhất của thiết bị.
    """
    try:
        # 1. Thông số độc nhất của Android (Settings Secure ID)
        # 🔑 Bắt buộc phải cài đặt gói termux-api
        android_info = "NO_ANDROID_ID" 
        try:
            # Chạy lệnh termux-telephony-deviceinfo
            result = subprocess.run(['termux-telephony-deviceinfo'], capture_output=True, text=True, timeout=5)
            
            # 💡 FIX: Lấy toàn bộ output để đảm bảo tính duy nhất
            if result.stdout and len(result.stdout) > 10:
                android_info = result.stdout.strip()
            
        except Exception:
            # Fallback nếu lệnh không chạy được
            pass

        # 2. Lấy đường dẫn Home (Vẫn giữ lại để tăng tính ổn định nếu android_info lỗi)
        home_path = os.path.expanduser('~') 

        # 3. Kết hợp các yếu tố và băm (Hash)
        raw_hwid_string = f"{home_path}-{android_info}-TERMUX_V7_HARDWARE_FIX" 
